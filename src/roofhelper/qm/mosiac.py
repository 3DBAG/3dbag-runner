...
def get_extent(tiff_path: str) -> tuple[float, float, float, float] | None:
...
def get_extent_ignore_nodata(tiff_path: str) -> tuple[float, float, float, float]:
    ...
    with rasterio.open(tiff_path) as src:
        ...
        for _, window in src.block_windows():
            ...
            # Mask nodata values
            data_array_masked = np.ma.masked_equal(data_array, nodata)  # type: ignore[no-untyped-call]
...
def calculate_extent(tiff_file: str, ignore_nodata: bool) -> tuple[float, float, float, float] | None:
...
def create_json(input_folder: str, output_json: dict[str, Any], ignore_nodata: bool, overwrite_path: str | None) -> None:
    ...
    for filename in os.listdir(input_folder):
        if filename.lower().endswith((".tif", ".tiff")):
            ...
            extent_result = calculate_extent(tiff_file, ignore_nodata)
            if extent_result is None:
                continue
            left, bottom, right, top = extent_result
